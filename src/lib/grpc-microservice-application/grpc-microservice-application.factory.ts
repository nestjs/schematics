...
export function main(options: GrpcMicroserviceApplicationOptions): Rule {
  options = transform(options);
  return mergeWith(generate(options));
}

function transform(options: GrpcMicroserviceApplicationOptions): GrpcMicroserviceApplicationOptions {
  const target: GrpcMicroserviceApplicationOptions = Object.assign({}, options);
...
function generate(options: GrpcMicroserviceApplicationOptions): Source {
