...
export function main(options: MqttMicroserviceApplicationOptions): Rule {
  options = transform(options);
  return mergeWith(generate(options));
}

function transform(options: MqttMicroserviceApplicationOptions): MqttMicroserviceApplicationOptions {
  const target: MqttMicroserviceApplicationOptions = Object.assign({}, options);
...
function generate(options: MqttMicroserviceApplicationOptions): Source {
