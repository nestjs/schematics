...
export function main(options: AMQPMicroserviceApplicationOptions): Rule {
  options = transform(options);
  return mergeWith(generate(options));
}

function transform(options: AMQPMicroserviceApplicationOptions): AMQPMicroserviceApplicationOptions {
  const target: AMQPMicroserviceApplicationOptions = Object.assign({}, options);
...
function generate(options: AMQPMicroserviceApplicationOptions): Source {
