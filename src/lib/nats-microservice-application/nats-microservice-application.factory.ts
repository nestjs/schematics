...
export function main(options: NATSMicroserviceApplicationOptions): Rule {
  options = transform(options);
  return mergeWith(generate(options));
}

function transform(options: NATSMicroserviceApplicationOptions): NATSMicroserviceApplicationOptions {
  const target: NATSMicroserviceApplicationOptions = Object.assign({}, options);
...
function generate(options: NATSMicroserviceApplicationOptions): Source {
