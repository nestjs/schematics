...
const SCHEMATICS_NAME: string = 'redis-microservice-application';

describe('Redis Microservice Application Factory', () => {
  const runner: SchematicTestRunner = new SchematicTestRunner(
    '.',
    path.join(process.cwd(), 'src/collection.json'),
  );
  it('should manage name only', () => {
    const options: RedisMicroserviceApplicationOptions = {
      name: 'project',
    };
  ...
  it('should manage name to dasherize', () => {
    ...
    expect(files).toEqual([
      ...
      '/awesome-project/test/mocks/redis.ts',
    ]);
  });
