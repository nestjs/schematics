import { WebSocketGateway<% if (crud === 'yes') { %>, SubscribeMessage, MessageBody<% } %> } from '@nestjs/websockets';
...
export class <%= classify(name) %>Gateway {
  constructor(private readonly <%= lowercased(name) %>Service: <%= classify(name) %>Service) {}<% if (crud === 'yes') { %>
