...
export class <%= classify(name) %>Gateway {
  ...
  async create(@MessageBody() create<%= singular(classify(name)) %>Dto: Create<%= singular(classify(name)) %>Dto) {
  ...
  async update(@MessageBody() update<%= singular(classify(name)) %>Dto: Update<%= singular(classify(name)) %>Dto) {
