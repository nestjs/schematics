import { Resolver<% if ((crud === 'yes' || crud === 'prisma') && type === 'graphql-schema-first') { %>, Query, Mutation, Args<% } else if ((crud === 'yes' || crud === 'prisma') && type === 'graphql-code-first') { %>, Query, Mutation, Args, Int<% } %> } from '@nestjs/graphql';
import { <%= classify(name) %>Service } from './<%= name %>.service';<% if ((crud === 'yes' || crud === 'prisma') && type === 'graphql-code-first') { %>
import { <%= singular(classify(name)) %> } from './entities/<%= singular(name) %>.entity';<% } %><% if ((crud === 'yes' || crud === 'prisma')) { %>
...
export class <%= classify(name) %>Resolver {
  constructor(private readonly <%= lowercased(name) %>Service: <%= classify(name) %>Service) {}<% if ((crud === 'yes' || crud === 'prisma') && type === 'graphql-code-first') { %>
  ...
  }<% } else if ((crud === 'yes' || crud === 'prisma') && type === 'graphql-schema-first') {%>
