...
<% if (crud) { %>export const CommandHandlers = [
...
@Module({
  <% if (type === 'cqrs') { %>imports: [
    <% if (crud) { %>TypeOrmModule.forFeature([<%= singular(classify(name)) %>]),
    <% } %>CqrsModule,
  ...
  providers: [<%= classify(name) %>Service]<% } else if (type === 'graphql-code-first' || type === 'graphql-schema-first') { %>providers: [<%= classify(name) %>Resolver, <%= classify(name) %>Service]<% } else if (type === 'cqrs') { %>controllers: [<%= classify(name) %>Controller],
    providers: [<%= classify(name) %>Service, <%= singular(classify(name)) %>Sagas,<% if (crud) { %> ...CommandHandlers, ...EventHandlers,<% } %> ...Consumers]<% } else { %>providers: [<%= classify(name) %>Gateway, <%= classify(name) %>Service]<% } %>
})
